...
export function getGeometries(geometries) {
  return geometries.map(function (item) {
    return _objectSpread(_objectSpread({}, item), {}, {
      center: new TMap.LatLng(item.center.lat, item.center.lng)
    });
  });
}
var MultiCircleComponent = /*#__PURE__*/React.forwardRef(function (props, ref) {
  var map = useContext(MapContext); // 获取地图实例
  var _useState = useState(),
    _useState2 = _slicedToArray(_useState, 2),
    instance = _useState2[0],
    setInstance = _useState2[1]; // 存储圆形图层实例
  ...
  /** 初始化图层 */
  var initMultiCircle = function initMultiCircle() {
    if (!map) return;
    // 图层初始化代码
    var multiCircleInstance = new TMap.MultiCircle(_objectSpread({
      id: id,
      map: map,
  ...
  // @hook 监听样式改变
  useEffect(function () {
    instance === null || instance === void 0 || instance.setStyles(getStyle('circle', styles));
  }, [styles]);
